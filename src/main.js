...
export function main() {
    ...
    function updateURL() {
        ...
        if (
            CONFIG.radarProductTime !== null &&
            CONFIG.radarProduct !== null &&
            CONFIG.radar !== null
        ) {
            url += `/radar/${CONFIG.radar}-${
                CONFIG.radarProduct
        ...
        for (let i = 1; i + 1 < pathSegments.length; i += 2) {
            if (pathSegments[i] === 'event') {
                ...
                if (radartokens.length === 3) {
                    ...
                    CONFIG.radarProductTime = moment.utc(
                        escapeHTML(radartokens[2]),
                        'YYYYMMDDHHmm'
                    );
                }
    ...
    function loadTabs() {
        ...
        $.ajax({
            ...
            success: (data) => {
                ...
                tabcontent.empty();
                tabs.append(
                    '<li><a href="#tall" data-toggle="tab">All</a></li>'
                );
                const stamp = moment
                ...
                if (CONFIG.activeUpdate !== null) {
                    $(
                        `#textdata a[data-update="${CONFIG.activeUpdate}"]`
                    ).click();
                }
                ugcTable.clear();
